...
public class MCMAQuestion extends MCQuestion{
	...
	public void getAnswerFromStudent()
	{
		...
		int nA = 1;
		try {
			nA = Integer.parseInt(ScannerFactory.getKeyboardScanner().nextLine());
		}
		catch (Exception e)
		{
		...
		while(nA != 0) {
			...
			sA = ScannerFactory.getKeyboardScanner().nextLine();
			while (!isAnswered) {
				for (int i = 0; i < answers.size(); i++)
				{
					MCAnswer thisans = answers.get(i);
					if (thisans.getText().equalsIgnoreCase(sA))
					{
	...
	public void saveStudentAnswer(PrintWriter pw)
	{
		...
		for (int i = 0; i < studentAnswer.size(); i++)
		{
			pw.println(studentAnswer.get(i).getText());
		}
		pw.flush();
	}
